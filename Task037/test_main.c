...
int check_output(const char *input, double expected, int test_number)
{
  ...
  {
    printf("Test %d failed (expected: %.0f; got: %.0f)\n", test_number, expected, result);
    return 0;
  }
}
...
int main(void)
{
  ...
  passed += check_output("0", 1.0, 1);
  passed += check_output("1", 1.0, 2);
  passed += check_output("5", 120.0, 3);
  passed += check_output("10", 3628800.0, 4);
  passed += check_output("-1", -1.0, 5);
