...
Для чтения одного символа применяется функция getc() (функция fgetc() аналогична. Первая С-шный макрос, вторая - С-шная функция). Она имеет следующий прототип:
...
Возьмите код из предыдущего задания и измените его так, чтобы он работал применяя функции `getc()` и `putc()`.
